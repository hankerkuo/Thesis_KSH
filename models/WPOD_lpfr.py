...
def end_block(x):
    xprobs = Conv2D(1, 3, activation='sigmoid', padding='same')(x)
    lp_bbox = Conv2D(8, 3, activation='linear', padding='same')(x)
    fr_bbox = Conv2D(8, 3, activation='linear', padding='same')(x)
    prob_fr = Conv2D(3, 3, activation='softmax', padding='same')(x)
    return Concatenate(3)([xprobs, lp_bbox, fr_bbox, prob_fr])
