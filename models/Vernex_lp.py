...
def create_hourglass_network_vernex_lp(num_classes, num_stacks, num_channels, inres, outres, bottleneck):
    ...
    for i in range(num_stacks):
        head_next_stage, head_to_loss, head_lpd = hourglass_module(head_next_stage, num_classes, num_channels, bottleneck, i)
...
def hourglass_module(bottom, num_classes, num_channels, bottleneck, hgid):
    ...
    head_next_stage, head_parts, head_lpd = create_heads(bottom, rf1, num_classes, hgid, num_channels)

    return head_next_stage, head_parts, head_lpd
...
def create_heads(prelayerfeatures, rf1, num_classes, hgid, num_channels):
    ...
    head_lpd_vertex = Conv2D(8, kernel_size=(3, 3), activation='linear', padding='same',
                             name=str(hgid) + '_conv_3x3_affine')(head)
    head_lpd = Concatenate(3)([head_lpd_prob, head_lpd_vertex])
